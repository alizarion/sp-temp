...
gulp.task('fonts', function() {
    gulp.src('src/assets/lib/**/*.{eot,svg,ttf,otf,woff,woff2}')
        .pipe(flatten())
...
gulp.task('locales', function() {
    gulp.src('./src/assets/locale/*/*.json')
...
gulp.task('clean-trad-fr', function () {
    return gulp.src('./src/assets/locale/fr/**.json',
        {force: true})
        .pipe(clean());
});
...
gulp.task('clean-trad-en', function () {
    return gulp.src('./src/assets/locale/en/**.json',
        {force: true})
        .pipe(clean());
});
...
gulp.task('build-trad-fr-only', function () {
    return gulp.src([ '!./src/assets/locale/**-fr.json',
        ...
        .pipe(gulp.dest('./src/assets/locale/fr'));
});
...
gulp.task('build-trad-en-only', function () {
    return gulp.src([ '!./src/assets/locale/**-fr.json',
        ...
        .pipe(gulp.dest('./src/assets/locale/en'));
});
...
gulp.task('test', function() {
    ...
    allVendorFiles.push('./src/assets/lib/angular-mocks/angular-mocks.js');
    var allAppFiles = buildConfig.appFiles.slice();
    allAppFiles = _removeValueFromArray(allAppFiles,'!src/app/**/*Test.js');
